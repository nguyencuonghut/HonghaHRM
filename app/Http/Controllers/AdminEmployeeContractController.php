...
class AdminEmployeeContractController extends Controller
{
    ...
    public function terminateForm ($contract_id)
    {
        $file_name = $this->makeSampleTerminateForm($contract_id);
    ...
    public function export ($contract_id)
    {
        ...
        switch ($employee_contract->contract_type_id) {
            case 1: // HĐ thử việc
                ...
                break;
            case 2: //HĐ lao động
                $file_name = $this->makeSampleHdld($contract_id);
                Alert::toast('Tải file thành công!!', 'success', 'top-right');
                return response()->download($file_name)->deleteFileAfterSend(true);
    ...
    private function makeSampleHdld($contract_id)
    {
        $employee_contract = EmployeeContract::findOrFail($contract_id);
        $employee = Employee::findOrFail($employee_contract->employee_id);
        ...
        if ($employee_contract->end_date) {
            $diff_months = Carbon::parse($employee_contract->end_date)->diffInMonths(Carbon::parse($employee_contract->start_date));
        ...
        if ($employee_contract->end_date) {
            $objBold = $objRichText->createTextRun(date('d/m/Y', strtotime($employee_contract->start_date)) . ' đến ' . date('d/m/Y', strtotime($employee_contract->end_date)));
        } else {
            // Không thời hạn
            $objBold = $objRichText->createTextRun(date('d/m/Y', strtotime($employee_contract->start_date)));
        }
        $objBold->getFont()->setBold(true);
        $objBold->getFont()->setName("Times New Roman");
        ...
        $employee_work = EmployeeWork::where('employee_id', $employee->id)->orderBy('id', 'desc')->first();
        $objBold = $objRichText->createTextRun($employee_work->company_job->name);
        $objBold->getFont()->setBold(true);
        $objBold->getFont()->setName("Times New Roman");
        ...
        $employee_work = EmployeeWork::where('employee_id', $employee->id)->orderBy('id', 'desc')->first();
        $objBold = $objRichText->createTextRun($employee_work->company_job->department->name);
        $objBold->getFont()->setBold(true);
        $objBold->getFont()->setName("Times New Roman");
    ...
    private function makeSampleHdtv($contract_id)
    {
        $employee_contract = EmployeeContract::findOrFail($contract_id);
        $employee = Employee::findOrFail($employee_contract->employee_id);
        // Make new sheet
        ...
        $objBold = $objRichText->createTextRun('từ ngày ' . date('d/m/Y', strtotime($employee_contract->start_date))
                                                            .' đến ngày '
    ...
    private function makeSampleTerminateForm($contract_id)
    {
        $employee_contract = EmployeeContract::findOrFail($contract_id);
        $employee = Employee::findOrFail($employee_contract->employee_id);
        ...
        // Số hợp đồng
        $w_sheet->mergeCells("A5:D5");
        $w_sheet->setCellValue('A5', 'Số: ' . $employee->code .'/' .  date('Y', strtotime($employee_contract->end_date)) .'/QĐ-HH');
        $w_sheet->getStyle("A5")->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);

        // Thời gian
        $w_sheet->mergeCells("E5:J5");
        $w_sheet->setCellValue('E5', 'Hà Nam, ngày ' . date('d/m/Y', strtotime($employee_contract->end_date)));
        $w_sheet->getStyle("E5")->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        ...
        $w_sheet->getStyle("A12")->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
        $w_sheet->setCellValue('A12', '(Về việc: Chấm dứt hợp đồng lao động với ông/bà ' .  $employee_contract->employee->name . ')');
        ...
        $w_sheet->setCellValue('A18', '- Căn cứ Hợp đồng lao động số: ' . $employee_contract->code . ' ký ngày ' . date('d/m/Y', strtotime($employee_contract->start_date)) . ' giữa Công ty và Người lao động;');
        ...
        $w_sheet->setCellValue('B24', 'Chấm dứt Hợp đồng lao động đối với ông/bà ' . $employee_contract->employee->name . ' , chức danh: ' . $employee_contract->company_job->name . ' thuộc ' . $employee_contract->company_job->department->name . ' kể từ ngày ' . date('d/m/Y', strtotime($employee_contract->end_date)) . '.');
        ...
        $w_sheet->setCellValue('B30', 'Phòng HCNS, Phòng kế toán, các phòng liên quan và ông/bà ' . $employee_contract->employee->name . ' có trách nhiệm thi hành quyết định này.');

        $w_sheet->setCellValue('B32', 'Quyết định này có hiệu lực kể từ ngày ' . date('d/m/Y', strtotime($employee_contract->end_date)) . '.');
