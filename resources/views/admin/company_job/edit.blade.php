    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                    <form class="form-horizontal" method="post" action="{{ route('admin.company_jobs.update', $company_job->id) }}" name="edit_company_jobs" id="edit_company_jobs" enctype="multipart/form-data" novalidate="novalidate">
                        {{ csrf_field() }}
                        @method('PATCH')
                        <!-- /.card-header -->
                        <div class="card-body">
                                            ...
                                            <select name="sel_department" id="sel_department" data-placeholder="Chọn phòng ban" class="form-control select2" style="width: 100%;">
                                                ...
                                                @foreach($departments as $department)
                                                    <option value="{{$department->id}}"
                                                        @if($department->id == $company_job->department_id)
                                                            selected="selected"
                                                        @endif>
                                                        {{$department->name}}
                                                    </option>
                                            ...
                                            <select name="sel_division" id="sel_division" data-placeholder="Chọn bộ phận" class="form-control select2" style="width: 100%;">
                                                ...
                                                @foreach($divisions as $division)
                                                    <option value="{{$division->id}}"
                                                        @if($division->id == $company_job->division_id)
                                                            selected="selected"
                                                        @endif>
                                                        {{$division->name}}
                                                    </option>
