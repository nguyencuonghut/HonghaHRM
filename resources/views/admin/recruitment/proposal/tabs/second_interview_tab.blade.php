...
<!-- Phỏng vấn lần 2 -->
<div class="tab-pane fade" id="custom-tabs-one-profile-6" role="tabpanel" aria-labelledby="custom-tabs-one-profile-tab-6">
      ...
      || ($second_interview_invitation && 'Từ chối' != $second_interview_invitation->feedback))
        <div class="card card-secondary">
            ...
            <div class="card-body">
                ...
                <a href="#second_interview_detail{{' . $proposal_candidate->id . '}}" class="btn btn-success" data-toggle="modal" data-target="#create_second_interview_detail{{$proposal_candidate->id}}"><i class="fas fa-plus"></i></a>
                <br>
                <br>
                @endcan
                        ...
                        @if ($second_interview_details)
                          ...
                          @php
                            $action = '<a href="#edit_second_interview_detail{{' . $second_interview_detail->id . '}}" class="btn btn-success btn-sm" data-toggle="modal" data-target="#edit_second_interview_detail' . $second_interview_detail->id. '"><i class="fas fa-edit"></i></a>
                                    <form style="display:inline" action="'. route("admin.recruitment.second_interview_detail.destroy", $second_interview_detail->id) . '" method="POST">
                          ...
                          <tr>
                            ...
                            <form class="form-horizontal" method="post" action="{{ route('admin.recruitment.second_interview_detail.update', $second_interview_detail->id) }}" name="update_second_interview_detail" id="update_second_interview_detail" novalidate="novalidate">
                                @method('PATCH')
                                {{ csrf_field() }}
                          ...
                          <tr>
                            @php
                              ...
                              if ($second_interview_details) {
                                $action = '<a href="#create_second_interview_result" class="btn btn-success btn-sm" data-toggle="modal" data-target="#create_second_interview_result' . $proposal_candidate->id. '"><i class="fas fa-plus"></i></a>';
                                if ($second_interview_result) {
                                    $action = '<a href="#edit_second_interview_result{{' . $proposal_candidate->id . '}}" class="btn btn-success btn-sm" data-toggle="modal" data-target="#edit_second_interview_result' . $proposal_candidate->id. '"><i class="fas fa-edit"></i></a>
                                        <form style="display:inline" action="'. route("admin.recruitment.second_interview_result.destroy", $proposal_candidate->id) . '" method="POST">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="submit" name="submit" onclick="return confirm(\'Bạn có muốn xóa?\');" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></button>
                                        <input type="hidden" name="_token" value="' . csrf_token(). '"></form>';
                                }
                            ...
                            <form class="form-horizontal" method="post" action="{{ route('admin.recruitment.second_interview_result.store', $proposal_candidate->id) }}" name="create_second_interview_result" id="create_second_interview_result" novalidate="novalidate">
                                {{ csrf_field() }}
                                <div class="modal fade" tabindex="-1" id="create_second_interview_result{{$proposal_candidate->id}}">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4>Phỏng vấn lần 2: {{$candidate->name}}</h4>
                            ...
                            <form class="form-horizontal" method="post" action="{{ route('admin.recruitment.second_interview_result.update', $proposal_candidate->id) }}" name="edit_second_interview_result" id="edit_second_interview_result" novalidate="novalidate">
                                {{ csrf_field() }}
                                @method('PATCH')
                                <div class="modal fade" tabindex="-1" id="edit_second_interview_result{{$proposal_candidate->id}}">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4>Phỏng vấn lần 2: {{$candidate->name}}</h4>
        ...
        <form class="form-horizontal" method="post" action="{{ route('admin.recruitment.second_interview_detail.store', $proposal_candidate->id) }}" name="create_second_interview_detail" id="create_second_interview_detail" novalidate="novalidate">
            {{ csrf_field() }}
            <div class="modal fade" tabindex="-1" id="create_second_interview_detail{{$proposal_candidate->id}}">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4>Phỏng vấn lần 2: {{$candidate->name}}</h4>
